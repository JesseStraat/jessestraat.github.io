...
- Overleaf is online-only: if it is ever offline ([which it frequently is](https://status.overleaf.com/uptime)), you can't access your work. You don't want this to happen on the day of your project deadline. [Overleaf has even been DoS'ed in the past](https://x.com/overleaf/status/1761459759595540646).
...
- [Slower than TeX Live](https://github.com/MiKTeX/miktex/issues/22), especially in big beamer files.
- Only really maintained by a single person (Christian Schenk).
- There was at one point a huge bug that broke MiKTeX after updating. [The only way to solve it was by updating MiKTeX using the command line.](https://tex.stackexchange.com/a/718928) To my knowledge, fresh installations of MiKTeX should never encounter the bug any more, so it should be fine.
- Doesn't ship with Perl, so if you want to use `latexmk`, you have to install it manually (Windows users can do so [here](https://strawberryperl.com)).
...
Use [LTeX+](https://ltex-plus.github.io/ltex-plus/) for a grammar/spell checker compatible with LaTeX. It does have a lot of false positives though, so make sure to hide them regularly (you can find issues in the problems tab of VSCode).

I sometimes use the [GitHub Actions](https://marketplace.visualstudio.com/items?itemName=GitHub.vscode-github-actions) plugin to manage my workflow, but since I figured out [how to compile LaTeX files in a GitHub action](https://github.com/JesseStraat/ThesisTemplate/blob/master/.github/workflows/compile.yml), I haven't touched my workflows.

[Todo Tree](https://marketplace.visualstudio.com/items?itemName=Gruntfuggly.todo-tree) is an amazing extension. You can add to-dos in a comment, and the extension makes a list of all of them. Very useful for large projects.

[Licenser](https://marketplace.visualstudio.com/items?itemName=ymotongpoo.licenser) is a niche extension that automatically adds licences to your files. It's very situational and not for everyone.

My favourite extension is [Prettify Symbols Mode](https://marketplace.visualstudio.com/items?itemName=siegebell.prettify-symbols-mode), since it's able to visually replace certain phrases in your code with custom Unicode. For example, it might replace `\mathbb{N}` by `ℕ` in your code, or `\alpha` by `α`. Amazing, right? I no longer use it because I would regularly get harassed by my peers for it. Use at your own risk.
...
My recommendation for reference management is [Zotero](https://www.zotero.org). It has all the above features and more, including a browser extension. Make sure to install the [Better BibTeX](https://retorque.re/zotero-better-bibtex/) plugin for enhanced LaTeX support, and the [ZotMoov](https://github.com/wileyyugioh/zotmoov) plugin for moving `.pdf` files outside the Zotero cloud (its cloud storage is very limited).
...
- Other journals don't allow `.bbl` files, and require you to put the bibliography directly into your `.tex` document using `bibitem`s. In `natbib`, this is as easy as copying and pasting the contents of your generated `.bbl` file. In `biblatex`, you need the `biblatex2bibitem` package.
